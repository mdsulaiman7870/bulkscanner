...
def bulk_ip_scanner(request):
    ...
    if request.user.is_authenticated:
        ...
        if request.method == 'POST':
                ...
                for i in ip_add:
                    ...
                    if ipcheck is True:
                        ...
                        if not ip_addresses.objects.filter(ip=i).exists() and not failed_scanned_ip.objects.filter(ip=i).exists():
                            ...
                            response = requests.request(method='GET', url=abuseipdburl, headers=abuseipdbheaders, params=querystring)

                            decodedResponse = response.json()

                            if not 'error' in response.json(): 

                                isPublic = decodedResponse.get("data").get("isPublic")
                                ipVersion = decodedResponse.get("data").get("ipVersion")
                                isWhitelisted = decodedResponse.get("data").get("isWhitelisted")
                                abuseConfidenceScore = decodedResponse.get("data").get("abuseConfidenceScore")
                                countryCode = decodedResponse.get("data").get("countryCode")
                                usageType = decodedResponse.get("data").get("usageType")
                                isp = decodedResponse.get("data").get("isp")
                                domain = decodedResponse.get("data").get("domain")
                                totalReports = decodedResponse.get("data").get("totalReports")
                                numDistinctUsers = decodedResponse.get("data").get("numDistinctUsers")
                                lastReportedAt = decodedResponse.get("data").get("lastReportedAt")
                                hostnames = decodedResponse.get("data").get("hostnames")
                                ...
                                if erro_status == 429:
                                    ...
                                    return render(request, 'abuseipdb/ip_scanned_result.html', {'ip_scanned_result': ip_scanned_result, 'ip_flag_for_run_another_scan':ip_flag_for_run_another_scan })
            ...
            return render(request, 'abuseipdb/ip_scanned_result.html', {'ip_scanned_result': ip_scanned_result, 'ip_malicious_found':ip_malicious_found, 'ip_flag_for_run_another_scan':ip_flag_for_run_another_scan })

        else:
            abuseipdb_api_data = abuseipdb_apis.objects.all()
            columns_data = columns.objects.all()

            context = {'abuseipdb_api_data': abuseipdb_api_data, 'columns_data': columns_data}
...
def scan_single_ip(request):
    ...
    if request.user.is_authenticated:
        ...
        if request.method == 'POST':
            ...
            ipcheck = ipaddress.ip_address(ip_address).is_private

            if ipcheck is True:
                messages.info(request, ip_address + " is a private IP.")
                return redirect("ip_scanner")
                ...
                if not ip_addresses.objects.filter(ip=ip_address).exists() and not failed_scanned_ip.objects.filter(ip=ip_address).exists():
                    ...
                    if not 'error' in response.json(): 
                        ...
                        elif erro_status == "NotFoundError":
                    ...
                    messages.info(request, "IP already already exists in the table.")
                    return redirect("abuseipdb_scan_single_ip")

            return render(request, 'abuseipdb/ip_scanned_result.html', {'ip_scanned_result': ip_scanned_result, 'ip_malicious_found': ip_malicious_found})
...
def abuseipdb_malicious_ips(request):
    ...
    if request.user.is_authenticated:
        ...
        queryset = ip_addresses.objects.filter(Q(abuseConfidenceScore__gt=0) | Q(totalReports__gt=0)).all()
        ...
        if show_entries:
            ...
            context = {'all_malicious_ips': queryset,
                       'ips_count': ips_count}
        # ips_count = ip_addresses.objects.filter(Q(abuseConfidenceScore__gt=0) | Q(totalReports__gt=0)).count()
        # context = {'all_malicious_ips': queryset,
        #             'ips_count': ips_count}
...
def search_malicious_ips(request):
    ...
    if request.user.is_authenticated:
        ...
        ip_search_result = ip_addresses.objects.filter(Q(abuseConfidenceScore__gt=0) | Q(totalReports__gt=0), ip__contains=search_text)

        context = {'ip_search_result': ip_search_result, 'search_text':search_text}
...
def search_all_ips(request):
    ...
    if request.user.is_authenticated:
        ...
        ip_search_result = ip_addresses.objects.filter(ip__contains=search_text).all()
